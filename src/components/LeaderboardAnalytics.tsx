...
const SEED_MONTHLY: RankEntry[] = SEED_WEEKLY.map((e, i) => ({
  ...
  prevRank: Math.max(
    1,
    Math.min(20, e.rank + (i % 3 === 0 ? 1 : i % 3 === 1 ? -1 : 0)),
  ),
}));
...
function RankRow({ entry }: { entry: RankEntry }) {
  ...
  return (
    <div
      className={`flex items-center gap-3 px-4 py-3 border-b border-[#2d2d34] transition-colors ${rowBase} ${rowHover}`}
    >
      {/* ── Rank number ── */}
      ...
      <div className="flex-1 min-w-0">
        <p
          className={`text-sm font-semibold leading-snug truncate ${isHigh ? "text-white" : "text-slate-200"}`}
        >
          {entry.title}
...
export default function LeaderboardAnalytics({
  ...
  const fetchRankings = async (type: "weekly" | "monthly") => {
    setLoading(true);
    setError(null);
    try {
      const res = await apiClient.rankings.getAll(type);
  ...
  const normalCount = entries.filter(
    (e) => getTier(e.rank) === "normal",
  ).length;
  ...
  const handleIngest = async () => {
    ...
    try {
      ...
      const ratingsList = Array.isArray(allRatings)
        ? allRatings
        : (allRatings as any)?.data || [];
      const voteTotals: Record<string, number> = {};
      for (const r of ratingsList) {
        voteTotals[r.seriesId] =
          (voteTotals[r.seriesId] || 0) + (r.voteCount || 0);
      }
  ...
  const parseCSV = (text: string) => {
    ...
    const titleIdx = headers.findIndex(
      (h) =>
        h.includes("title") || h.includes("seriesid") || h.includes("name"),
    );
    const votesIdx = headers.findIndex(
      (h) =>
        h.includes("vote") ||
        h.includes("count") ||
        h.includes("rank") ||
    ...
    const sourceIdx = headers.findIndex(
      (h) => h.includes("source") || h.includes("period"),
    );

    if (titleIdx === -1 || votesIdx === -1) {
      throw new Error(
        "Invalid CSV format. Must contain columns: 'title' or 'seriesId' and 'votes' or 'voteCount'.",
      );
    }
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const sourceVal =
        sourceIdx !== -1 && parts[sourceIdx] ? parts[sourceIdx] : "Weekly";
          ...
          {view === "board" && (
            <button
              onClick={() => {
                ...
                setIngestMsg("");
                setIngestSeriesId("");
                setIngestVoteCount(15000);
                setIngestSource("Weekly");
              }}
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {!loading && !error && (
          <div className="flex items-center gap-3 px-4 py-2 mt-4 bg-[#181820] border-y border-[#2d2d34] select-none">
            <div className="w-7 shrink-0 text-[8px] font-bold text-slate-600 uppercase tracking-widest text-center">
              #
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-[#1e1e24] border border-[#2d2d34] rounded-md shadow-2xl shadow-black w-full max-w-md overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-[#2d2d34]">
              ...
              <button
                onClick={() => setShowIngestModal(false)}
                className="p-1 rounded-md hover:bg-[#2d2d34] transition-colors cursor-pointer"
              >
            ...
            <div className="p-5 space-y-4">
              {ingestMsg && (
                <div
                  className={`px-4 py-2.5 rounded-md border text-[11px] font-bold ${ingestMsg.startsWith("🚀") ? "bg-green-600/10 border-green-600/30 text-green-400" : "bg-red-600/10 border-red-600/30 text-red-400"}`}
                >
                  {ingestMsg}
                </div>
              )}
