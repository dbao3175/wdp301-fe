...
export default function EditorialBoard({
  ...
  const handleAssignVoters = async () => {
    ...
    try {
      ...
      setVoterStatus(statusRes || null);
      setSelectedVoterIds([]);
      setModalMessage(`🎉 ${t('Voters assigned successfully!')}`);
      fetchAllProposals();
    } catch (err: any) {
      setModalMessage(`❌ ${t(err.message)}`);
    }
                ...
                <div key={item._id} className="bg-white border-4 border-ink-black border-t-0 first:border-t-4 p-5 flex items-center gap-5 hover:bg-manuscript-gray transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="font-sans text-[10px] font-bold text-neutral-500 uppercase mt-0.5">
                      {t("Author")}: {prop.mangakaId ? (prop.mangakaId as any).name || 'Mangaka' : 'Mangaka'}
                    </p>
            ...
            <div className="motion-modal-panel relative bg-white border-4 border-ink-black shadow-[8px_8px_0px_#141414] w-full max-w-2xl max-h-[85vh] overflow-y-auto">
              <div className="sticky top-0 bg-white border-b-4 border-ink-black p-5 flex items-start justify-between z-10">
                <div>
                  ...
                  <p className="font-sans text-[10px] font-bold text-neutral-500 uppercase mt-0.5">
                    {t('Author')}: {typeof selectedProposal.mangakaId === 'object' && selectedProposal.mangakaId !== null ? (selectedProposal.mangakaId as any).name : 'Mangaka'}
                  </p>
                </div>
              ...
              <div className="p-5 space-y-5">
                ...
                <div className="bg-manuscript-gray p-4 border-2 border-ink-black space-y-3">
                  ...
                  >
                    {t('Assign Voters')} ({selectedVoterIds.length} {t('selected')})
                  </button>
                </div>
                )}
